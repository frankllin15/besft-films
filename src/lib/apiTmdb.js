...
export const getMultipleMediaById = async (list) => {
    ...
    const data = Promise.all(list.map(async ({id, media_type}) => {
...
export const getPeapleById = async (id) => {
    ...
    return fetch(`${process.env.NEXT_PUBLIC_BASE_API}/person/${id}?api_key=${process.env.NEXT_PUBLIC_TMDB_KEY}&language=pt-BR`)
        .then(data => data.json())
}

export const getArtWorks = async (id, type, page) => {
    page = Number(page) -1
    return fetch(`${process.env.NEXT_PUBLIC_BASE_API}/person/${id}/${type}_credits?api_key=${process.env.NEXT_PUBLIC_TMDB_KEY}&language=pt-BR`)
    .then(data => data.json())
    .then(({cast}) => {

        const maxPage = Math.floor(cast.length/20)
        const results = cast.filter((e, id) => id >= page * 20  && id <= (page+1)*20-1)
