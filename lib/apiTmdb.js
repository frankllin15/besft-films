...
export const getMediaByGenre = async (type, genreId) => {
    return fetch(`${process.env.NEXT_PUBLIC_BASE_API}/discover/${type}?api_key=${process.env.NEXT_PUBLIC_TMDB_KEY}&language=pt&sort_by=popularity.desc&include_adult=false&include_video=true&page=1&with_genres=${genreId}`)
        .then(resp => resp.json())
